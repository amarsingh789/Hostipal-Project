...
const PatientDashboard = () => {
  ...
  const upcomingApts = appointments.filter((a) => {
    ...
    return a.status !== "Cancelled" && a.status !== "Completed" && aptDate >= todayDate;
  });
    ...
    <div className="min-h-screen bg-[#F6F9F8] font-inter pb-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            <div className="w-full md:w-auto text-center md:text-left">
              <motion.p
                variants={fadeUp}
                className="text-[#dfff4f] text-xs md:text-sm font-semibold tracking-wider uppercase mb-2 flex items-center justify-center md:justify-start gap-2"
              >
                <Calendar size={14} /> {today}
              </motion.p>
              <motion.h1
                variants={fadeUp}
                className="text-3xl sm:text-4xl md:text-5xl font-poppins font-bold text-white mb-2"
              >
                Hello, {user?.name ? user.name.split(" ")[0] : "User"}
              </motion.h1>
              <motion.p
                variants={fadeUp}
                className="text-[#a8cfc3] text-sm md:text-base"
              >
            ...
            <motion.div
              variants={fadeUp}
              className="flex w-full md:w-auto justify-center gap-3 md:gap-4 mt-4 md:mt-0"
            >
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center text-white transition-colors relative shrink-0"
              >
                <BellRing size={20} />
              ...
              >
                <Link
                  to="/appointment"
                  className="w-full md:w-auto justify-center bg-[#dfff4f] text-[#021814] font-bold font-poppins px-6 py-3 rounded-full shadow-[0_4px_20px_rgba(223,255,79,0.2)] flex items-center gap-2 group"
                >
                  Book Visit{" "}
                  <ChevronRight
                    size={18}
          ...
          >
            ...
            <motion.div
              variants={fadeUp}
              whileHover={{ y: -5 }}
              className="lg:col-span-1 bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl md:rounded-3xl p-5 md:p-6 flex flex-row lg:flex-col justify-between items-center lg:items-stretch transition-shadow hover:shadow-[0_8px_30px_rgba(15,118,110,0.2)]"
            >
              <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center w-full lg:mb-4 gap-1">
                ...
                <button
                  ...
                  className="text-[#dfff4f] hover:text-white transition-colors text-xs font-bold uppercase tracking-wider w-max"
                >
                ...
                {vitalsData.score ? (
                  <>
                    <motion.span
                      ...
                      className="text-4xl md:text-5xl font-poppins font-bold text-white leading-none"
                    >
                      {vitalsData.score}
                    </motion.span>
            ...
            <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <motion.div
                  ...
                  className="bg-white/5 border border-white/10 rounded-2xl md:rounded-3xl p-5 flex items-center gap-4 transition-all hover:bg-white/10"
                >
                  <div
                    className={`w-10 h-10 md:w-12 md:h-12 shrink-0 ${stat.bg} ${stat.color} rounded-full flex items-center justify-center`}
                  >
                    <stat.icon size={20} />
                  </div>
                  <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 -mt-6 md:-mt-8 relative z-20">
        ...
        >
          ...
          <motion.div
            variants={fadeUp}
            className="lg:col-span-2 bg-white rounded-2xl md:rounded-[2rem] shadow-sm border border-gray-100 p-5 md:p-8"
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
              ...
              <div className="flex p-1 bg-gray-100 rounded-xl w-full sm:w-max overflow-hidden relative">
                <button
                  ...
                  className={`relative z-10 flex-1 sm:flex-none px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-semibold transition-colors ${activeTab === "upcoming" ? "text-[#0F766E]" : "text-gray-500 hover:text-gray-700"}`}
                >
                  {activeTab === "upcoming" && (
                    <motion.div
                      layoutId="tab-pill"
                ...
                <button
                  ...
                  className={`relative z-10 flex-1 sm:flex-none px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-semibold transition-colors ${activeTab === "past" ? "text-[#0F766E]" : "text-gray-500 hover:text-gray-700"}`}
                >
                  {activeTab === "past" && (
                    <motion.div
                      layoutId="tab-pill"
            ...
            {loading ? (
              <div className="py-12 text-center flex flex-col items-center justify-center text-gray-400 text-sm">
                <Loader2 size={30} className="animate-spin mb-2" /> Loading
                schedule...
              </div>
              ...
              <motion.div
                ...
                className="py-12 md:py-16 text-center border-2 border-dashed border-gray-100 rounded-2xl"
              >
                  ...
                  >
                    {displayApts.map((apt, idx) => {
                      ...
                      const isCancelledOrExpired =
                        apt.status === "Cancelled" || isExpired;

                      const displayStatus = isExpired ? "Expired" : apt.status;
                        ...
                        >
                          ...
                          <div className="mt-4 pt-3 md:pt-4 border-t border-gray-100 flex flex-wrap items-center justify-between gap-2">
                            ...
                            {isCancelledOrExpired ? (
                              <Badge
                                variant="destructive"
                                className="bg-red-50 text-red-600 border-none font-bold gap-1 text-[10px] md:text-xs hover:bg-red-50"
                              >
                                <XCircle size={12} /> {displayStatus}
                              </Badge>
                            ) : (
                              <Badge className="bg-[#E6F4F1] text-[#0F766E] border-none font-bold gap-1 text-[10px] md:text-xs hover:bg-[#E6F4F1]">
                            ...
                            <div className="flex flex-wrap gap-1 md:gap-2">
                              ...
                              {!isCancelledOrExpired && activeTab === "upcoming" && (
                                <>
                                  <button
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent className="rounded-2xl md:rounded-[2rem] w-[90vw] md:w-auto max-w-md">
                                      <AlertDialogHeader>
                                        <AlertDialogTitle className="font-poppins text-[#021814]">
                                          Cancel Appointment?
                                        </AlertDialogTitle>
                                        <AlertDialogDescription className="font-inter text-sm">
                                          This will permanently cancel your
                                          appointment. Note: Cancellations are
          ...
          <div className="flex flex-col gap-6 md:gap-8">
            <motion.div
              variants={fadeUp}
              className="bg-white rounded-2xl md:rounded-[2rem] shadow-sm border border-gray-100 p-5 md:p-8"
            >
            ...
            <motion.div
              variants={fadeUp}
              className="bg-white rounded-2xl md:rounded-[2rem] shadow-sm border border-gray-100 p-5 md:p-8 overflow-hidden"
            >
