...
const TeamPage = () => {
  ...
  const swiperReveal = {
    hidden: { opacity: 0, x: 50 }, // Comes slightly from the right
    visible: { 
      opacity: 1, 
      x: 0, 
    ...
    <div className="w-full py-28 bg-white overflow-hidden">
      ...
      >
        <div className="max-w-2xl">
          <motion.div variants={slideUpItem} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-clinic-green/5 border border-clinic-green/10 mb-6">
            <Award size={18} className="text-clinic-green" />
            <span className="text-xs font-bold text-clinic-green uppercase tracking-widest">
              Medical Experts
            </span>
          </motion.div>
          <motion.h2 variants={slideUpItem} className="text-5xl md:text-6xl font-black font-poppins text-gray-900 leading-tight">
            Meet our <br />
            <span className="text-clinic-green">Specialists</span>
          </motion.h2>
        </div>
        <motion.p variants={slideUpItem} className="text-gray-500 text-lg font-inter max-w-md md:text-right">
      ...
      <motion.div 
        variants={swiperReveal}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, margin: "-50px" }}
        className="w-full pl-6 md:pl-10 lg:pl-[calc((100vw-1280px)/2+24px)]"
      >
        <Swiper 
          slidesPerView={1.2}
          spaceBetween={24}
