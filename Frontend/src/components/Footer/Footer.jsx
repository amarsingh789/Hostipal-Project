...
const Footer = () => {
  ...
  // Comming soon handler for non-functional links
  const handleComingSoon = (e, featureName) => {
    e.preventDefault();
    toast(`${featureName} is coming soon!`, {
      icon: "🚧",
      style: { borderRadius: "12px", background: "#021814", color: "#dfff4f" },
    });
  };
  return (
    <footer className="w-full bg-[#021814] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#032b24] to-[#021814] text-white pt-24 pb-8 px-6 lg:px-12 font-inter border-t border-white/5 overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col justify-between h-full relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 pb-24">
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            <div className="flex items-center gap-2 text-clinic-yellow mb-2">
              <Activity size={24} />
              <span className="text-xl font-black font-poppins tracking-tight text-white">
                ZIVA.
              </span>
            </div>
            <h4 className="text-sm font-bold font-poppins text-white/40 uppercase tracking-[0.2em]">
              Explore
            </h4>
            <div className="flex flex-col gap-3">
              ...
              <Link
                to="/doctor"
                className="text-white/70 hover:text-clinic-yellow hover:translate-x-2 transition-all duration-300 text-sm font-medium w-fit flex items-center gap-2 group"
              >
              ...
              {/* Assuming services are on the home page or about page */}
              <Link
                to="/about"
                className="text-white/70 hover:text-clinic-yellow hover:translate-x-2 transition-all duration-300 text-sm font-medium w-fit flex items-center gap-2 group"
              >
              ...
              <Link
                to="#"
                onClick={(e) => handleComingSoon(e, "Ziva Blog")}
                className="text-white/70 hover:text-clinic-yellow hover:translate-x-2 transition-all duration-300 text-sm font-medium w-fit flex items-center gap-2 group"
              >
                <span className="w-0 h-[1px] bg-clinic-yellow transition-all duration-300 group-hover:w-3"></span>
                Ziva Blog
          ...
          {/* col-02 */}
          <div className="lg:col-span-3 flex flex-col gap-6">
            <h4 className="text-sm font-bold font-poppins text-white/40 uppercase tracking-[0.2em] mt-10 md:mt-0">
              Departments
            </h4>
            <div className="flex flex-col gap-3">
              {[
                ...
                <Link
                  to="#"
                  key={dept}
                  onClick={(e) => handleComingSoon(e, dept)}
                  className="text-white/70 hover:text-clinic-yellow hover:translate-x-2 transition-all duration-300 text-sm font-medium w-fit flex items-center gap-2 group"
                >
                  <span className="w-0 h-[1px] bg-clinic-yellow transition-all duration-300 group-hover:w-3"></span>
                  {dept}
            ...
            <div className="flex flex-col gap-5 mt-2">
              ...
              >
                ...
                <p className="text-sm leading-relaxed pt-1">
                  123 Healing Boulevard, <br /> Medical District, City 40001
                </p>
              </Link>
              <a
                href="tel:+918009482223"
              ...
              <a
                href="mailto:support@zivahealth.com"
                className="flex items-center gap-4 text-white/70 hover:text-white transition-colors group"
              >
                <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-clinic-yellow group-hover:text-[#021814] transition-all duration-300">
                  <Mail size={18} />
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            <p className="text-white/60 text-sm leading-relaxed">
              Subscribe for the latest health tips and exclusive wellness
              offers.
            </p>
            <form
            ...
            >
              <div className="relative">
                <input
                  ...
                  className="bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-clinic-yellow transition-colors w-full peer"
                />
                <button
                  type="submit"
                  className="absolute right-2 top-2 bottom-2 bg-clinic-yellow text-[#021814] px-4 rounded-xl font-bold hover:bg-white transition-colors flex items-center justify-center"
                >
                  <ArrowRight size={18} />
                </button>
              </div>
            ...
            <div className="flex items-center gap-3 mt-4">
              {[Instagram, Twitter, Linkedin].map((Icon, idx) => (
                <a
                  href="#"
                  key={idx}
                  className="w-10 h-10 rounded-full border border-white/30 flex items-center justify-center text-white/60 hover:bg-clinic-yellow hover:text-[#021814] hover:border-clinic-yellow hover:-translate-y-1 transition-all duration-300"
                >
                  <Icon />
                </a>
              ))}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mt-12 pt-6 border-t border-white/10 text-white/40 text-xs w-full">
          <p>
            © {new Date().getFullYear()} Ziva Healthcare. All rights reserved.
          </p>
          <div className="flex gap-6 mt-4 md:mt-0 font-medium">
            <Link
              to="#"
              onClick={(e) => handleComingSoon(e, "Privacy Policy")}
              className="hover:text-clinic-yellow transition-colors"
            >
              Privacy Policy
            </Link>
            <Link
            ...
            <Link
              to="#"
              onClick={(e) => handleComingSoon(e, "Cookie Settings")}
